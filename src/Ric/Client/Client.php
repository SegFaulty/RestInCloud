					...
					if( $serverMissingVersions ){
						...
						foreach( $serverMissingVersions as $serverMissingVersion ){
							if( $pushMissingVersions ){
								if( !in_array($primaryFileVersion, $versions) and $primaryFileTimestamp>time() - 3600 ){ // we dont fix it the file is fresh (<1h)
...
#			$response .= print_r($knownFilesPerServer, true);
	...
	/**
	 * @param string $pattern // full regex pattern to match file names
